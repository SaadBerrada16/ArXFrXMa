...
private:
    ...
    void save_in_order(BSTNode<T> * node, std::vector<T>& vector, int& i);
...
public:
    ...
    // smallest value to the greatest value.
    void save_in_order(std::vector<T>& vector);
...
template <class T>
void BST<T>::save_in_order(BSTNode<T>* node, std::vector<T>& vector, int& i)
{
    if (node != NULL)
    {
...
template <class T>
void BST<T>::save_in_order(std::vector<T>& vector)
{
    int i = 0;
